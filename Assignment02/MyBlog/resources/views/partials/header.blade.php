    ...
    <div class="container mx-auto px-4 py-4 md:py-0  flex justify-between items-center">
        ...
        <nav class="hidden md:flex space-x-8 items-center">
            ...
            @auth
                ...
                <a href="/categories" class="text-gray-600 hover:text-blue-600 transition">Category List</a>
                <a href="/blogs" class="text-gray-600 hover:text-blue-600 transition">Blog List</a>
            @endauth
            <!-- Categories Dropdown -->
                    ...
                    <div class="py-2">
                        @foreach($categories as $category)
                            <a href="{{ route('category.blogs', $category->id) }}"
                                class="block px-4 py-2 text-gray-700 hover:bg-gray-100 transition">{{ $category->name }}</a>
        ...
        <div class="flex items-center space-x-4">
            ...
            @auth
                <form action="{{ route('logout') }}" method="POST" class="inline">
                    @csrf
                    <button type="submit"
                        class="hidden md:block bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 transition">
                        Logout
                    </button>
                </form>
            @endauth
